...
5. **2:20–2:50 — Sponsor workflow and safety.** Send a real coordination request; Firecrawl and AgentMail are live on production. Run a real pre-screen: Firecrawl fetches the source and the model returns advisory findings against named gate keys. Finish with the safety boundary: no AI result and no demo status grants permission to cut.
...
npx convex run diagnostics:prescreenProbe '{"permitId":"<id>","sourceUrl":"<url>"}' --prod
